...
class ContiguousVideoClipDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            # 2020/09/04 Titi
            eleven_range = tuple([full_range[0], full_range[1] - self.seq_len + 1])
            possible_index_list = []
            for i in range(200):
                possible_index = i * 11
                if (possible_index > eleven_range[1]):
                    break
                if (possible_index >= eleven_range[0]):
                    possible_index_list.append(possible_index)
            if (len(possible_index_list) == 0):
                if not self.resample_on_fail:
                    raise RuntimeError('Interval %s in video %s Possible_index_list length=0' % (str(full_range), video_file_path))
                    
                index = np.random.randint(0, len(self.files))
                continue
            random.shuffle(possible_index_list)
            start_index = possible_index_list[0]
